...
The solver cannot write to any file except standard output, standard error and files in the TMPDIR directory. 
Solvers can use several processes or threads. 
** Output Rules
...
Lines output by the solver should be prefixed by 'c ', 's ', 'v ', 'd ' or 'o '. 
...
**** objective cost ('o ' line) (for optimization only)
...
** solver.sh
   A shell script compatible with the execution environment.
** Instances
   An arborescence which contains all instances.
** Configurations
A flat directory which contains different configuration for the solver.
* Benchmarking
* Reporting
